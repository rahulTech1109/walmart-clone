...
async function ProductPage({ searchParams: { url } }: Props) {
    ...
    return (
        <div className="p-4 lg:p-10 flex flex-col lg:flex-row w-full">
            <div className="hidden lg:inline space-y-4">{product.images.map((image, i) => (
                <Image key={image} src={image} alt={product.title + i} width={90} height={90} className="border rounded-sm" />
            ))}</div>
            <Carousel className="w-3/5 mb-10 lg:mb-0 lg:w-full self-start flex items-center max-w-xl  mx-auto lg:mx-20" opts={{ loop: true }}>
                <CarouselContent>
                    {product.images.map((image, i) => (<CarouselItem key={i}>
                        <div className="p-1">
            ...
            <div className="flex-1 border rounded-md w-full p-5 space-y-5">
                ...
                <div className="space-x-2">{product.breadcrumbs.map((breadcrum, i) => (
                    <Badge key={breadcrum + i} className={breadcrum} variant="outline">{breadcrum}</Badge>
                ))}
                </div>
